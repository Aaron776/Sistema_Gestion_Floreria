...
foreach ($categorias as $cat) {
    $filas .= '
            <tr>
                <td class="td-num">' . $cat->id_categoria . '</td>
                <td>' . htmlspecialchars($cat->nombre) . '</td>
                <td>' . htmlspecialchars($cat->descripcion ?: '—') . '</td>
                <td class="td-num">' . (int) $cat->total_productos . '</td>
                <td class="td-num">' . (int) $cat->stock_total . '</td>
                <td class="td-num">$' . number_format((float) $cat->valor_inventario, 2) . '</td>
            </tr>';
}
if ($filas === '') {
    $filas = '
            <tr><td colspan="6" class="td-empty">No hay categorías registradas.</td></tr>';
}

$fecha_generacion = date('d/m/Y H:i');
$promedio = $resumen->total_categorias > 0 ? $total_productos->total / $resumen->total_categorias : 0;
$promedio_fmt = number_format((float) $promedio, 1);
...
<body>
    ...
    <table class="header">
        <tr>
            ...
            <td class="title-cell">
                <div class="title-report">REPORTE DE CATEGORÍAS</div>
    ...
    <table class="summary">
        <tr>
            <td><div class="box"><div class="box-label">Categorías</div><div class="box-value">{$resumen->total_categorias}</div></div></td>
            <td><div class="box"><div class="box-label">Productos totales</div><div class="box-value">{$total_productos->total}</div></div></td>
            <td><div class="box"><div class="box-label">Promedio productos/categoría</div><div class="box-value">{$promedio_fmt}</div></div></td>
        </tr>
    </table>

    <div class="section-title">Categorías y su rendimiento</div>
    <table class="data">
        <thead>
            <tr>
                ...
                <th>Descripción</th>
                <th style="width: 12%;" class="td-num">Productos</th>
                <th style="width: 12%;" class="td-num">Stock total</th>
                <th style="width: 15%;" class="td-num">Valor inventario</th>
            </tr>
        </thead>
        <tbody>$filas
        </tbody>
    ...
    <div class="footer">
        Reporte de categorías generado electrónicamente por el sistema · $fecha_generacion
    </div>
</body>
</html>
HTML;
...
$dompdf->render();
$dompdf->stream('reporte_categorias.pdf', ['Attachment' => true]);
