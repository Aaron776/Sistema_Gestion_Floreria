...
// Consultas SQL
try {
  // Total de pedidos en camino asignados a este repartidor
  $sql_total = $conexion->prepare("SELECT COUNT(*) FROM entregas WHERE id_repartidor = :id_repartidor AND estado = 'en_camino'");
  $sql_total->bindParam(":id_repartidor", $id_repartidor, PDO::PARAM_INT);
  $sql_total->execute();
  $total_registros = $sql_total->fetchColumn();

  // Total pedidos pendientes asignados a este repartidor
  $sql_pendientes = $conexion->prepare("SELECT COUNT(*) FROM entregas WHERE id_repartidor = :id_repartidor AND estado = 'pendiente'");
  $sql_pendientes->bindParam(":id_repartidor", $id_repartidor, PDO::PARAM_INT);
  $sql_pendientes->execute();
  $total_pendientes = $sql_pendientes->fetchColumn();
...
<style>
    ...
    .btn-change-status:hover {
      ...
      box-shadow: 0 4px 12px rgba(74, 122, 92, 0.25);
    }
  ...
  <div class="admin-container">
    ...
    <header class="admin-header">
      <div class="title-section">
        <h1><i class="fas fa-truck" style="color:#4a7a8a; margin-right:0.5rem;"></i>Pedidos en Camino</h1>
        <span class="badge">En camino</span>
      </div>
      <div class="header-actions">
        <a href="gestion_pedidos_listos.php" class="btn-outline">
          <i class="fas fa-box"></i> Pedidos listos
        </a>
      </div>
    </header>
    ...
    <div class="table-container">
      <div class="table-header">
        <div class="table-title">
          <i class="fas fa-list" style="color:#4a7a8a; margin-right:0.5rem;"></i>
          Lista de pedidos <span>(<?php echo $total_registros; ?> en camino)</span>
        </div>
      </div>
      ...
      <table>
        ...
        <tbody id="ordersTableBody">
          <?php if (!empty($pedidos_en_camino) && count($pedidos_en_camino) > 0): ?>
            <?php foreach ($pedidos_en_camino as $item): ?>
              <tr data-status="en_camino">
                ...
                <td style="text-align:center;">
                  <form action="../controladores/repartidor/confirmar_entrega.php" method="post" class="form-change-status">
  ...
  <script>
    ...
    function filterTable() {
      ...
      rows.forEach(row => {
        ...
        const cliente = row.querySelector('td:nth-child(2) div')?.textContent?.toLowerCase() || '';
    ...
    // ===== CONFIRMAR ENTREGA CON SWEETALERT2 =====
