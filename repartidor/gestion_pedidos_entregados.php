...
// Consultas SQL
try {
  // Total de registros con estado entregado
  $sql_total = $conexion->prepare("SELECT COUNT(*) FROM pedidos WHERE estado='entregado'");
  $sql_total->execute();
  ...
  // Traer pedidos con estado entregado paginados
  $sql = $conexion->prepare("SELECT p.direccion_entrega as direccion, p.id_pedido as id_pedido, cliente.nombre as cliente, cliente.email as email, vendedor.nombre as vendedor, p.fecha_pedido as fecha_pedido, p.total as total FROM pedidos p JOIN usuarios as cliente ON p.id_cliente = cliente.id_usuario JOIN usuarios as vendedor ON p.id_usuario = vendedor.id_usuario WHERE p.estado='entregado' ORDER BY p.fecha_pedido DESC LIMIT :limite OFFSET :offset");
  $sql->bindValue(':limite', $resultados_por_pagina, PDO::PARAM_INT);
  $sql->bindValue(':offset', $offset, PDO::PARAM_INT);
  $sql->execute();
  $pedidos_entregados = $sql->fetchAll(PDO::FETCH_OBJ);

} catch (PDOException $e) {
  error_log("Error en las consultas a la base de datos: " . $e->getMessage());
  $pedidos_entregados = [];
  $total_registros = 0;
  $total_paginas = 1;
  $pagina_actual = 1;
...
<style>
  ...
  <div class="admin-container">
    ...
    <header class="admin-header">
      <div class="title-section">
        <h1><i class="fas fa-check-circle" style="color:#4a7a5c; margin-right:0.5rem;"></i>Gestión de Pedidos Entregados</h1>
    ...
    <div class="table-container">
      <div class="table-header">
        <div class="table-title">
          <i class="fas fa-list" style="color:#4a7a5c; margin-right:0.5rem;"></i>
          Lista de pedidos <span>(<?php echo $total_registros; ?> entregados en total)</span>
        </div>
      </div>
      ...
      <table>
        ...
        <tbody id="ordersTableBody">
          <?php if (!empty($pedidos_entregados) && count($pedidos_entregados) > 0): ?>
            <?php foreach ($pedidos_entregados as $item): ?>
              <tr data-status="entregado">
          ...
          <?php else: ?>
            <tr>
              <td colspan="6" style="text-align:center; padding:2rem; color:#7f6e5d;">
  ...
  <script>
    ...
    function changeStatus(idPedidoEncrypted) {
      // Función para cambiar estado si se requiere modal o redirección
      console.log('Cambiar estado del pedido:', idPedidoEncrypted);
    }
  </script>
